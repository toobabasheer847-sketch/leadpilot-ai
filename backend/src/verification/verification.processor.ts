...
export class VerificationProcessor extends WorkerHost {
  ...
  process(job: Job<VerificationJobData>) {
    this.logger.info('job.verification.started', { jobId: job.id, companyId: job.data.companyId, contactId: job.data.contactId });
    return this.service.verifyQueued(job.data).catch((error: unknown) => {
      this.logger.warn('job.verification.failed', { jobId: job.id, companyId: job.data.companyId, errorType: error instanceof Error ? error.name : 'unknown' });
      throw error;
    });
  }
}
