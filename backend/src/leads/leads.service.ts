...
export class LeadsService {
  ...
  async listExecutions(user: AuthenticatedUser, filters: ListExecutionsDto) {
    ...
    return { data: executions.map((item) => ({ id: item.execution.id, userPrompt: item.prompt, status: item.execution.status, createdAt: item.execution.createdAt, completedAt: item.execution.completedAt, resultCount: countMap.get(item.execution.id) ?? 0 })), pagination: { page: filters.page, limit: filters.limit, total: Number(total), totalPages: Math.ceil(Number(total) / filters.limit) } };
  }
